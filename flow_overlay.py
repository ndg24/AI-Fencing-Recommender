...
def writeOpticalFlowToVideo(video_string):
    ...
    frame1 = np.concatenate(
        (
            frame1[height_start:height_end, 0:width, 0:depth],
            frame1[height_end + height // 14 - 10 : height, 0:width, 0:depth],
        ),
        axis=0,
    )
    ...
    clip_height = frame1.shape[0]
    output_file = (
        "optical_flow/" + video_string.replace("final_training_clips/", "").replace(".mp4", "") + "move" + ".mp4"
    )
    ...
    while subcount <= frames_till_video_end - last_frame:
        ...
        frame2 = np.concatenate(
            (
                frame2[height_start:height_end, 0:width, 0:depth],
                frame2[height_end + height // 14 - 10 : height, 0:width, 0:depth],
            ),
            axis=0,
        )
...
os.makedirs("preinception_data", exist_ok=True)
os.makedirs("final_training_clips/already_optical_flowed", exist_ok=True)
...
if train_set is not None:
    ...
    print("No clips found in final_training_clips/")
